        ...
        if (mode == LNumberType.NumberMode.MODE_NUMBER && number == (float) Math.round(number)) {
            ...
            if (Float.isInfinite(number)) {
                ...
                if (PrintFlag.test(flags, PrintFlag.DISASSEMBLER)) {
                    var bits = Float.floatToRawIntBits(number);
                    var canonical = Float.floatToRawIntBits(Float.NaN);
                    if (bits == canonical) {
                        return "NaN";
                    } else {
                        var sign = "+";
    ...
    public String toPrintString(int flags) {
        if (mode == LNumberType.NumberMode.MODE_NUMBER && number == (double) Math.round(number)) {
            ...
            if (Double.isInfinite(number)) {
                ...
                if (PrintFlag.test(flags, PrintFlag.DISASSEMBLER)) {
                    var bits = Double.doubleToRawLongBits(number);
                    var canonical = Double.doubleToRawLongBits(Double.NaN);
                    if (bits == canonical) {
                        return "NaN";
                    } else {
                        var sign = "+";
