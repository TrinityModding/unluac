...
import me.hydos.unluac.api.LuaCode;
...
public class NewDecompiler {
    ...
    public final LuaCode result = new LuaCode();
    ...
    public NewDecompiler(BFunction target, List<Declaration> parentDeclarations, int startLine) {
        ...
        var luaMajor = bytecode.header.version.getVersionMajor();
        var luaMinor = bytecode.header.version.getVersionMinor();
        System.out.println("Lua Bytecode version " + luaMajor + "." + luaMinor);
    ...
    private void processSequence(State state, List<Block> blocks, int end) {
        ...
        while (true) {
            ...
            if (blockStack.peek().end <= line) {
                ...
                var operation = endingBlock.process();
            ...
            if (operations == null) {
                if (blockStatementIndex < blockStatements.size() && blockStatements.get(blockStatementIndex).begin <= line) {
                    ...
                    var operation = blockStatement.process();
